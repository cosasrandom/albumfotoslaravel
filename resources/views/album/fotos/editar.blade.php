<x-app-layout>
    ...
    <div class="py-8 px-4 sm:px-6 lg:px-8 bg-gradient-to-r from-pink-50 to-purple-50 border-b border-pink-100">
        <div class="max-w-3xl mx-auto text-center">
            ...
            <p class="mt-2 text-pink-400 italic">Actualiza los detalles de tu foto en el álbum: {{ $foto->album->nombre }}</p>
        </div>
    </div>
            ...
            <div class="bg-white bg-opacity-70 backdrop-blur-sm rounded-2xl shadow-xl overflow-hidden">
                <div class="p-6 md:p-8">
                    ...
                    <form method="POST" action="{{ route('foto.update', $foto) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-5">
                            ...
                            <input type="text" name="nombre" id="nombre" 
                                ...
                                value="{{ old('nombre', $foto->nombre) }}" required autofocus>
                            @error('nombre')
                        ...
                        <div class="mb-5">
                            ...
                            <textarea name="descripcion" id="descripcion" rows="3" 
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm 
                                       focus:border-pink-400 focus:ring focus:ring-pink-200 focus:ring-opacity-50
                                       transition duration-150 ease-in-out px-4 py-2">{{ old('descripcion', $foto->descripcion) }}</textarea>
                            @error('descripcion')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            <label for="foto" class="block text-sm font-semibold text-gray-700 mb-1">Cambiar Foto (Opcional)</label>
                            @if ($foto->foto_ruta)
                                <div class="mb-2">
                                    <p class="text-sm text-gray-600">Foto actual:</p>
                                    <img src="{{ asset('storage/' . $foto->foto_ruta) }}" alt="Foto actual" class="w-32 h-32 object-cover rounded-lg shadow-md mt-1">
                                </div>
                            @endif
                    ...
                    <div class="mt-8 text-center">
                        <a href="{{ route('foto.index', ['album_id' => $foto->album_id]) }}" class="inline-flex items-center px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-full text-sm font-semibold transition-colors duration-200">
